"""
Section 3 Master Generator: Units 83 - 92 (45 Lessons)
"""

import json
from scripts.framework_builder.gen_sec03_part1 import get_sec03_part1_lessons
from scripts.framework_builder.gen_sec03_part2 import get_sec03_part2_lessons
from scripts.framework_builder.gen_sec03_part3 import get_sec03_part3_lessons

def get_sec03_lessons():
    with open("curriculum/blueprint/units/a2.json", "r", encoding="utf-8") as f:
        units = json.load(f)
    u_map = {u["sequencePosition"]: u for u in units}

    lessons = []
    lessons.extend(get_sec03_part1_lessons(u_map))
    lessons.extend(get_sec03_part2_lessons(u_map))
    lessons.extend(get_sec03_part3_lessons(u_map))

    # Verify lexical budget for each unit in Section 3 (83-92)
    lessons_by_unit = {}
    for l in lessons:
        uid = l["unitId"]
...
if __name__ == "__main__":
    sec03 = get_sec03_lessons()
    print(f"Total Section 3 lessons: {len(sec03)}")
    with open("curriculum/lesson_blueprints/a2_section_03.json", "w", encoding="utf-8") as f:
        json.dump(sec03, f, ensure_ascii=False, indent=2)
    print("Successfully saved curriculum/lesson_blueprints/a2_section_03.json")
