"""
Section 4 Master Generator: Units 93 - 102 (49 Lessons)
"""

import json
from scripts.framework_builder.gen_sec04_part1 import get_sec04_part1_lessons
from scripts.framework_builder.gen_sec04_part2 import get_sec04_part2_lessons
from scripts.framework_builder.gen_sec04_part3 import get_sec04_part3_lessons

def get_sec04_lessons():
    with open("curriculum/blueprint/units/a2.json", "r", encoding="utf-8") as f:
        units = json.load(f)
    u_map = {u["sequencePosition"]: u for u in units}

    lessons = []
    lessons.extend(get_sec04_part1_lessons(u_map))
    lessons.extend(get_sec04_part2_lessons(u_map))
    lessons.extend(get_sec04_part3_lessons(u_map))

    # Verify lexical budget for each unit in Section 4 (93-102)
    lessons_by_unit = {}
    for l in lessons:
        uid = l["unitId"]
...
if __name__ == "__main__":
    sec04 = get_sec04_lessons()
    print(f"Total Section 4 lessons: {len(sec04)}")
    with open("curriculum/lesson_blueprints/a2_section_04.json", "w", encoding="utf-8") as f:
        json.dump(sec04, f, ensure_ascii=False, indent=2)
    print("Successfully saved curriculum/lesson_blueprints/a2_section_04.json")
